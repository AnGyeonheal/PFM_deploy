...
def growth_png(gdf):
    ...
    if "내 자산가치" in gdf:
        ax.plot(idx, gdf["내 자산가치"], color="#EF553B", lw=2.0, label="My Assets")
    if "S&P500 자산가치" in gdf:
        ax.plot(idx, gdf["S&P500 자산가치"], color="#636EFA", lw=1.6, ls="--", label="S&P500 (sell-adj)")
    if "순투자원금" in gdf:
        ax.plot(idx, gdf["순투자원금"], color="#9AA4AE", lw=1.2, ls=":", label="Net Principal")
    if "내 누적손익" in gdf:
        ax.fill_between(idx, gdf["내 누적손익"], color="#14B8A6", alpha=0.12)
        ax.plot(idx, gdf["내 누적손익"], color="#14B8A6", lw=1.4, label="Cumulative P&L")
    ax.set_title("Asset Growth vs S&P500 (KRW)", fontsize=11)
