...
type Stock = {
  ...
  currentPrice: number | null; avgPrice: number; holdingUnrealizedPnL: number | null; holdingReturnPct: number | null; status: string;
};
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/7">
                {["종목명", "티커", "통화", "수량", "평단가", "현재가", "보유분 평가손익", "보유분 수익률"].map(h => (
            ...
            <tbody>
              {stocks.map(s => (
                <tr key={s.ticker} className="border-b border-white/4">
                  ...
                  <td className={`px-2 py-1 font-mono text-xs text-end whitespace-nowrap ${s.holdingUnrealizedPnL == null ? "text-[#6b7494]" : s.holdingUnrealizedPnL >= 0 ? "text-[#00d4a1]" : "text-[#ff5c6a]"}`}>{formatKRW(s.holdingUnrealizedPnL, true)}</td>
                  <td className={`px-2 py-1 font-mono text-xs text-end whitespace-nowrap ${s.holdingReturnPct == null ? "text-[#6b7494]" : s.holdingReturnPct >= 0 ? "text-[#00d4a1]" : "text-[#ff5c6a]"}`}>{s.holdingReturnPct == null ? "—" : `${s.holdingReturnPct >= 0 ? "+" : ""}${s.holdingReturnPct.toFixed(2)}%`}</td>
                </tr>
              ))}
              {stocks.length === 0 && (
