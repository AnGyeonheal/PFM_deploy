...
- "증권사": "{broker_name}"
- "계좌": 원본의 계좌 식별자를 문자열로 보존(앞자리 0 유지). 없으면 빈 문자열, 추측 금지
...
6. 같은 종목이어도 서로 다른 계좌의 수량과 평균단가를 합치지 마세요.
...
5. 티커를 확실히 매핑하지 못해도 해당 거래 행을 생략하지 마세요. 티커는 빈 문자열로 두고 원본 종목명과 확인 가능한 필드는 그대로 반환하세요. 티커를 추측해 만들지 마세요.
...
각 원소(배당 1건)의 필드:
- "증권사": "{broker_name}"
- "계좌": 원본 계좌 식별자 문자열. 없으면 빈 문자열, 추측 금지
- "일자": 배당 입금일 "YYYY-MM-DD"
- "배당락일": 원본에 명시된 배당락일 "YYYY-MM-DD". 없으면 빈 문자열
- "기준일": 원본에 명시된 배당 기준일 "YYYY-MM-DD". 없으면 빈 문자열
...
6. 티커를 확실히 매핑하지 못해도 해당 배당 행을 생략하지 마세요. 티커는 빈 문자열로 두고 원본 종목명과 확인 가능한 필드는 그대로 반환하세요. 티커를 추측해 만들지 마세요.
7. 배당락일이나 기준일을 입금일로 대신 쓰지 마세요. 실제 입금일이 확인되지 않으면 일자는 빈 문자열로 반환하세요.
